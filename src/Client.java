...
public class Client {
     ...
     */
    private static String executeUdpRequest(InetAddress hostName, int serverPort, String reqCode) throws Exception {
        // Create a UDP socket
        ...
        if (acknowledgement.equals(NO)) {
            throw new Exception("Could not verify tcp port");
        }
        udpSocket.close();
     ...
     * @param msg
     * @return
     * @throws IOException Establishes TCP connection with the server and sends a string to the server. Receives reversed string from
     *                     the server and closes the connection
     */
    private static String executeTcpRequest(InetAddress hostName, int serverPort, String msg) throws IOException {
    ...
    /**
     * @param args Runs the client program
     */
    public static void main(String[] args) {
        ...
        try {
            ...
            String tcpPort = executeUdpRequest(hostName, serverPort, String.valueOf(reqCode));
            // Get the response from the server over tcp connection
            String revMsg = executeTcpRequest(hostName, Integer.valueOf(tcpPort), msg);
            // Print the response
